...
@Controller
@RequestMapping("ko")
public class KoIndexController {
	String languageType = "ko";
...
//		}
		...
		if(StringUtils.isBlank(sessionVerCode)||StringUtils.isBlank(verCode)){
			model.addAttribute("message", "조작과정에서 오차가있으니 다시 확인하십시오");
			return "forward:/"+languageType+"/register";
		}else if(!sessionVerCode.toLowerCase().equals(verCode.toLowerCase())){
			model.addAttribute("message", "인증번호 입력에 오차가 있으니 다시 확인하십시오");
			return "forward:/"+languageType+"/register";
		}
...
//		userInfo.setFileName(fileNames);
		...
		try {
			Assert.isNull(userInfo, "받는상대를 써넣으십시오");
			Assert.isNull(userInfo.getName(), "아이디를 써넣으십시오");
			Assert.isNull(userInfo.getPassword(), "비밀번호를 써넣으십시오");
			Assert.isNull(userInfo.getCellphone(), "핸드폰 번호를 써넣으십시오");
			Assert.isNull(userInfo.getIdNum(), "신분증번호를 써넣으십시오");
		} catch (Exception e) {
			e.printStackTrace();
    ...
    public String submit(HttpServletRequest request,HttpServletResponse response,Map<String, Object> map) {
        ...
        if(user==null){
        	...
        	map.put("msg", "아이디 또는 비밀번호가 정확하지 않습니다");
    ...
    public @ResponseBody String checkUser(HttpServletRequest req,UserInfo userInfo) {
    	...
    	try {
    		...
    		if(user != null && user.getName() != null){
    			throw new BizException("7000","이미 존재하는 아이디 입니다");
    		}
    	} catch (Exception e) {
    ...
    public @ResponseBody String userIDPhoto(HttpServletRequest req) {
    	...
    	try {
    		...
    		if(user==null) {
    			result.setMessage("로그인하신뒤 사용하십시오");
        		result.setCode("7000");
        		return JSONObject.toJSONString(result);
    		}
